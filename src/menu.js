...
import { loadServers, addServer, deleteServer, defaultSettings } from './servers.js';
...
import { apiSaveCharacter, apiDeleteCharacter } from './api.js';
...
// Серверы загружаются async из API
let servers = [];
...
export function showMainMenu(app, startGameCallback) {
  ...
  // Загружаем серверы из API асинхронно
  loadServers().then(list => { servers = list; }).catch(() => { servers = []; });
...
async function showServerMenu() {
  ...
  // Показываем лоадер пока грузим серверы
  const loadingEl = document.createElement('div');
  loadingEl.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#ffcc80;font-size:20px;';
  loadingEl.textContent = 'Загрузка серверов…';
  menuContainer.innerHTML = '';
  menuContainer.appendChild(loadingEl);
  ...
  if (btnCreate) {
    btnCreate.onclick = async () => {
      ...
      try {
        await addServer(name, { ...currentSettings });
        alert(`Сервер «${name}» создан!`);
        showServerMenu(); // Обновляем список
      } catch (e) {
        alert('Ошибка создания сервера: ' + e.message);
      }
    };
  }
...
function showCharacterEditor() {
  ...
  footer.querySelector('#ce-save').addEventListener('click', async () => {
    ...
    // Сохраняем на сервере если авторизованы
    if (window.currentUser?.userId) {
      try {
        await apiSaveCharacter({ ...saved, user_id: window.currentUser.userId });
      } catch (e) {
        console.warn('Не удалось сохранить персонажа на сервере:', e);
      }
    }
...
function showCharacterSelectScreen() {
  ...
  // Если авторизованы и есть персонажи на сервере — синхронизируем в localStorage
  if (window.currentUser?.characters?.length) {
    window.currentUser.characters.forEach(c => {
      saveCharacter(c); // обновляем локальную копию из серверных данных
    });
  }
    ...
    if (chars.length === 0) {
      ...
      chars.forEach(char => {
        ...
        delBtn.addEventListener('click', async () => {
          ...
          // Удаляем на сервере если авторизованы
          if (window.currentUser?.userId) {
            try { await apiDeleteCharacter(char.id); } catch (e) { console.warn('Ошибка удаления на сервере:', e); }
          }
