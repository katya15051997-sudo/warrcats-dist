// sparring.js
// Совместная тренировка двух игроков.
//
// Флоу:
// 1. Игрок А ПКМ по Игроку Б → "Пригласить на тренировку"
//    → на сервер уходит {type:'sparring_invite', to: idБ}
// 2. У Б над головой появляется надпись "Имя_А предлагает тренировку. Готовы? Y/N"
//    → Б нажимает Y / кликает по "Y" над головой
//    → на сервер {type:'sparring_accept', to: idА}
// 3. Сервер рассылает обоим {type:'sparring_start', a: idА, b: idБ}
// 4. Тренировка: шкала выносливости 100%, 1 удар (E) = −10%
//    Победитель: +5 xp. Проигравший: +2.5 xp. Оба: −5 энергии.
//    Работает только уклонение из баффов.

import * as PIXI from 'pixi.js';
import { send, getMyId } from '../net/network.js';
import { addXp, applyMoveEffects, getLearnedMoves, progressMoveTasks } from './xp-system.js';
import { gainEnergy } from './needs-system.js';
...
/*
  _sparring = {
    myStamina: 100,       // выносливость своего персонажа
    opponentStamina: 100, // выносливость противника (синхронизируется через WS)
    opponentId: string,
    opponentName: string,
    myBar: PIXI.Graphics,
    opponentBar: PIXI.Graphics,
    myLabel: PIXI.Text,
    opponentLabel: PIXI.Text,
...
// Ожидающее приглашение (для Б)
let _pendingInvite = null; // { fromId, fromName, label: PIXI.Text, keyHandler }
...
// Установить ссылку на спрайт своего персонажа (обновляется каждый кадр)
...
export function handleSparringInvite(fromId, fromName, myCharSprite) {
  ...
  // Кнопка Y над надписью — кликабельная
  const yBtn = new PIXI.Text('Y', {
    fontSize: 18, fill: 0x88ff88, fontFamily: 'Arial',
  ...
  const nBtn = new PIXI.Text('N', {
    fontSize: 18, fill: 0xff8888, fontFamily: 'Arial',
    stroke: 0x000000, strokeThickness: 3,
  });
  ...
  // Обновлять позицию метки каждый кадр
  if (_app) {
    const ticker = () => {
...
export function startSparring(opponentId, opponentName, opponentSprite) {
  ...
  const headerText = new PIXI.Text(
    ...
    {
      fontSize: 16, fill: 0xffffff, fontFamily: 'Arial',
      stroke: 0x000000, strokeThickness: 4, align: 'center',
    }
  );
  headerText.anchor.set(0.5, 0);

  // Позиция шапки — фиксированная на экране, обновляем через camera offset
  _positionHeader(headerText);
  _world.addChild(headerText);

  // Шкалы под персонажами
  const myBar       = _makeStaminaBar(0x9966ff); // фиолетовая
  const opponentBar = _makeStaminaBar(0xaa44ff);
  ...
  _sparring = {
    ...
    opponentName,
    myBar, opponentBar,
    myLabel, opponentLabel,
    headerText,
    opponentSprite,
    stunned: false,      // оглушён ли я (не могу бить)
    stunTimeout: null,
  };

  // Каждый кадр — обновляем позиции баров
  const ticker = () => _updateSparringBars();
  _app.ticker.add(ticker);
  _sparring._ticker = ticker;
}

function _positionHeader(text) {
  // Позиционируем в мировых координатах — будет двигаться вместе с камерой
  // Поэтому обновляем каждый кадр
  if (_myCharSprite) {
...
function _makeStaminaBar(color) {
...
function _makeBarLabel(name) {
  return new PIXI.Text(name + ' 100%', {
    fontSize: 11, fill: 0xffffff, fontFamily: 'Arial',
    stroke: 0x000000, strokeThickness: 2,
  });
}

function _updateSparringBars() {
  if (!_sparring) return;
...
// ── Удар в тренировке (вызывать вместо обычного удара когда _sparring активен) ─
...
export function sparringStrike() {
  ...
  // Применяем только уклонение у противника
  const learnedDodge = getLearnedMoves().filter(m => m.effect?.type === 'dodgeChance');
  const { dodged, stunMs } = applyMoveEffects(10, learnedDodge);

  if (dodged) {
    showToast('Противник уклонился!');
    return;
  }

  const dmg = STAMINA_PER_HIT;

  if (stunMs > 0) {
    // Я оглушён (контратака не применяется — только уклонение)
  }

  // Отправить удар сопернику
  send('sparring_hit', { damage: dmg });
}

// ── Получить удар от соперника ────────────────────────────────────────────────

export function handleSparringHit(damage) {
  if (!_sparring) return;
  _sparring.myStamina = Math.max(0, _sparring.myStamina - damage);

  if (_sparring.myStamina <= 0) {
    _endSparring(false); // я проиграл
  }
}

// Обновить выносливость соперника (сервер прислал что тот получил урон)
export function handleSparringOpponentHit(damage) {
  if (!_sparring) return;
  _sparring.opponentStamina = Math.max(0, _sparring.opponentStamina - damage);

  if (_sparring.opponentStamina <= 0) {
    _endSparring(true); // я выиграл
  }
}
...
function _endSparring(iWon) {
  ...
  addXp(xpGain);
  gainEnergy(-5);
  progressMoveTasks('sparring'); // прогресс задания "Потренироваться с котом N раз"
...
export function handleSparringEnd() {
  // Вызывается если сервер прислал принудительное завершение
  _cleanupSparring();
}
