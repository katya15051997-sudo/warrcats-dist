...
import { createCharacter } from '../character/character-save.js';
...
export function showCharacterEditor(initial = null, onSaved = null) {
  ...
  function catHue(v){ if(v<=70) return Math.round((v/70)*60); return Math.round(180+((v-70)/30)*60); }
  ...
  function catHueGrad(){
    const s=[]; for(let i=0;i<=10;i++) s.push('hsl('+catHue(i*10)+',60%,45%) '+i*10+'%');
  ...
  function eyeHueGrad(){
    const s=[]; for(let i=0;i<=24;i++){ const h=Math.round((i/24)*360),p=Math.round((i/24)*100); s.push('hsl('+h+',100%,50%) '+p+'%'); }
    return 'linear-gradient(to right,'+s.join(',')+')';
  }
  ...
  function ensurePreviewHost(){
    ...
    try {
      ...
      (async () => {
        ...
        ['Sit','sleep','eyes'].forEach(n => { const s=bodySpine.skeleton.findSlot(n); if(s) s.setAttachment(null); });
        eyesSpine.skeleton.slots.forEach(sl => { if(sl.data.name!=='eyes') sl.setAttachment(null); });
        ...
        console.warn('Не удалось создать живое превью:', err);
      ...
      console.warn('PIXI-превью недоступно:', err);
      const img = document.createElement('img');
  ...
  function renderTop(){
    ...
    if (st.tribeOpen) {
      ...
      const closeTribeHandler = (e) => {
        ...
        if (p && !p.contains(e.target) && trigger && !trigger.contains(e.target)) {
          st.tribeOpen = false;
          renderTop();
    ...
    if (st.editingName) {
      ...
      inp.addEventListener('input',   (e) => { st.name = e.target.value; });
      inp.addEventListener('blur',    () => { if (!st.name.trim()) st.name='Без имени'; st.editingName=false; renderTop(); });
      ...
      nameDiv.addEventListener('click', () => { st.editingName=true; renderTop(); });
      center.appendChild(nameDiv);
    }

    tribeBtn.addEventListener('click', (e) => { e.stopPropagation(); st.tribeOpen=!st.tribeOpen; renderTop(); });
    ...
    ageInp.step = '1';
    ageInp.value = st.age;
    ageInp.style.cssText = 'background:rgba(20,35,20,0.9);border:1px solid #8B5A2B;border-radius:6px;color:#ffcc80;font-size:13px;font-weight:bold;padding:3px 8px;outline:none;width:60px;text-align:center;-moz-appearance:textfield;';
    ...
    ageInp.addEventListener('input', (e) => {
      ...
      if (v > 180) v = 180;
      st.age = v;
      const newLocked = getLockedSize(v);
      if (newLocked !== null) {
        st.size = newLocked;
        const sizeSliderEl = center.querySelector('#ce-size');
        const sizeValEl    = center.querySelector('#ce-size-val');
        if (sizeSliderEl) { sizeSliderEl.value = newLocked; sizeSliderEl.disabled = true; sizeSliderEl.style.opacity='0.4'; sizeSliderEl.style.cursor='not-allowed'; }
        if (sizeValEl)    sizeValEl.textContent = newLocked.toFixed(2);
      } else {
        const sizeSliderEl = center.querySelector('#ce-size');
        if (sizeSliderEl) { sizeSliderEl.disabled = false; sizeSliderEl.style.opacity='1'; sizeSliderEl.style.cursor='pointer'; }
      }
    });
    ...
    sizeVal.id = 'ce-size-val';
    sizeVal.style.cssText = 'font-size:11px;color:#ffcc80;min-width:28px;';
    sizeVal.textContent = effSize.toFixed(2);
    sizeSlider.addEventListener('input', (e) => { st.size=Number(e.target.value); sizeVal.textContent=Number(e.target.value).toFixed(2); });
  ...
  function renderBodySliders(){
    ...
    function refreshBuildBtns(){
      ...
      [['lean','Lean'],['large','Massive'],['fat','Fat']].forEach(([id,lbl]) => {
        ...
        btn.textContent = lbl;
        btn.addEventListener('click', () => { st.build=id; refreshBuildBtns(); updatePreview(); });
  ...
  function buildSliderElements(container, secKey, appData, isEye, tab){
    ...
    configs.forEach(cfg => {
      ...
      const labelText = document.createElement('span'); labelText.textContent = cfg.label;
      const valSpan   = document.createElement('span'); valSpan.style.cssText='color:#ffcc80;font-weight:bold;'; valSpan.textContent=appData[cfg.key];
      labelRow.appendChild(labelText); labelRow.appendChild(valSpan);
      wrap.appendChild(labelRow);

      const slider = document.createElement('input');
      slider.type='range'; slider.min=cfg.min; slider.max=cfg.max; slider.step=1; slider.value=appData[cfg.key];
      ...
      slider.addEventListener('input', () => {
        appData[cfg.key] = Number(slider.value);
        valSpan.textContent = slider.value;
        container.querySelectorAll('input[type=range]').forEach(s => { s._refreshBg && s._refreshBg(); });
        if (tab==='body'||tab==='eyes') updatePreview();
      });
  ...
  footer.querySelector('#ce-save').addEventListener('click', async () => {
    let initialAge = parseInt(st.age, 10);
    if (Number.isNaN(initialAge) || initialAge < 0) initialAge = 0;
    if (initialAge > 180) initialAge = 180;
    ...
    banner.textContent = 'Котик «' + (saved.name ?? st.name) + '» сохранён!';
